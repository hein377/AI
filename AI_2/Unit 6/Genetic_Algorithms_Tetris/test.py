...
def heuristic(tup, strategy):
    ...
    print(f"num rows completed: {rows_completed}")
    print(f"max height: {20-maxrow}")
    print(f"num holes: {nh}")
    print(f"well_depth: {wd}")
    print(f"c_h: {col_heights}")
    print(f"heights_sum: {hs}")
    print(f"heuristic_score: {value}")
    print()
    input()
...
def best_board(pos_boards, strategy):
    ...
    print(f"BEST BOARD:")
    print(printboard(bestboard))
    print(f"c_h: {best_ch}      maxrow: {bestmaxrow}        num_rows_completed: {best_rcompleted}")
...
def play_game(strategy):                    #strategy is a set of numeric values (coefficients for heuristic)
    ...
    print(f"c_h: {col_heights}      board_max_row: {boardmaxrow}")
...
#Global Variables
POPULATION_SIZE = 300
NUM_VARIABLES = 6                           #number of weights for strategy
NUM_CLONES = 25
NUM_TRIALS = 5
TOURNAMENT_SIZE = 30
...
def create_nextgen(scored_pop, ranked_pop):
    nextgen = []
...
def algo(tup, generation):
    ...
    if(c == "C"): algo(process_pop(create_nextgen(scored_pop, ranked_pop)), generation+1)
...
print(play_game(tuple([random.uniform(-1, 1) for x in range(NUM_VARIABLES)])))

'''print("ORIGINAL BOARD:")
print(printboard(board))
c_h, bmaxrow = find_colheights_boardmaxrow(list(board))
print(c_h)
print(bmaxrow)
input()
for tup in pos_next_boards(list(board), "I1", bmaxrow, c_h): 
    print(tup)
    board, bmaxrow, ch, rows_completed = tup
    print(printboard(board))
    print(bmaxrow)
    print(ch)
    input()'''
